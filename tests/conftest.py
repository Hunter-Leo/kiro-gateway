...
def setup_test_environment(tmp_path_factory):
    ...
    original_app_creds_file = kiro.app.ACCOUNTS_CONFIG_FILE
    original_app_state_file = kiro.app.ACCOUNTS_STATE_FILE

    kiro.config.ACCOUNTS_CONFIG_FILE = str(creds_file)
    kiro.config.ACCOUNTS_STATE_FILE = str(tmp_dir / "state.json")
    # kiro.app binds these at import time, so patch it as well — otherwise the
    # lifespan writes credentials.json/state.json into the working directory.
    kiro.app.ACCOUNTS_CONFIG_FILE = str(creds_file)
    kiro.app.ACCOUNTS_STATE_FILE = str(tmp_dir / "state.json")
    ...
    kiro.app.ACCOUNTS_CONFIG_FILE = original_app_creds_file
    kiro.app.ACCOUNTS_STATE_FILE = original_app_state_file
