...
class TestLifespanLegacyFallback:
    ...
    async def test_lifespan_legacy_mode_recreate_credentials(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kiro.app.ACCOUNT_SYSTEM", False)
        monkeypatch.setattr("kiro.app.REFRESH_TOKEN", "test_refresh_token")
        monkeypatch.setattr("kiro.app.KIRO_CREDS_FILE", None)
        monkeypatch.setattr("kiro.app.KIRO_CLI_DB_FILE", None)
        ...
        # Mock config paths
        monkeypatch.setattr("kiro.app.ACCOUNTS_CONFIG_FILE", str(creds_file))
        monkeypatch.setattr("kiro.app.ACCOUNTS_STATE_FILE", str(state_file))
        ...
        with patch("kiro.app.AccountManager", return_value=mock_manager):
            with patch("kiro.app.httpx.AsyncClient") as mock_client_class:
                mock_client = AsyncMock()
                mock_client_class.return_value = mock_client
                
                # Import and run lifespan
                from kiro.app import lifespan, app
    ...
    async def test_lifespan_account_system_one_time_migration(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kiro.app.ACCOUNT_SYSTEM", True)
        monkeypatch.setattr("kiro.app.REFRESH_TOKEN", "test_refresh_token")
        monkeypatch.setattr("kiro.app.KIRO_CREDS_FILE", None)
        monkeypatch.setattr("kiro.app.KIRO_CLI_DB_FILE", None)
    ...
    async def test_lifespan_migration_priority_sqlite(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kiro.app.ACCOUNT_SYSTEM", True)
        monkeypatch.setattr("kiro.app.REFRESH_TOKEN", "test_refresh_token")
        monkeypatch.setattr("kiro.app.KIRO_CLI_DB_FILE", str(sqlite_db))
        monkeypatch.setattr("kiro.app.KIRO_CREDS_FILE", str(json_file))
...
class TestLifespanAccountManagerInit:
    ...
    async def test_lifespan_create_account_manager(self, tmp_path, monkeypatch):
        ...
        # Arrange: Patch constants
        monkeypatch.setattr("kiro.app.ACCOUNT_SYSTEM", True)
        monkeypatch.setattr("kiro.app.REFRESH_TOKEN", "test_token")
        monkeypatch.setattr("kiro.app.KIRO_CREDS_FILE", None)
        monkeypatch.setattr("kiro.app.KIRO_CLI_DB_FILE", None)
        ...
        with patch("kiro.app.AccountManager", MockAccountManager):
            with patch("kiro.app.httpx.AsyncClient") as mock_client_class:
                mock_client = AsyncMock()
                mock_client_class.return_value = mock_client
